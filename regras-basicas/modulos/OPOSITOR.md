...
Ative quando houver forças que:

```text
não possuem Cadeira própria
+
possuem objetivos, recursos ou planos próprios
+
continuam agindo além de uma reação local
+
se beneficiam de um executor estratégico separado do Narrador
```
...
criatura estratégica recorrente
```

Perigo ambiental, processo impessoal ou conflito entre personagens com Cadeira própria não exigem Opositor.
...
```text
definir ações das forças dentro de seu escopo
...
Quando for necessário preservar informação adversarial, usar:

```text
oposicao.md
```

Ele pode guardar objetivos, preparação, recursos, conhecimento, prazos e estágios de execução.

```text
PLANO REGISTRADO
≠ sucesso futuro garantido.
```

O plano pode ser descoberto, impedido, alterado, abandonado ou derrotado.

## Conhecimento

```text
EXECUTOR DO OPOSITOR SABE
≠ FORÇA ADVERSARIAL SABE
≠ PERSONAGEM SABE
```

Cada força age somente com seu próprio conhecimento legítimo.
