...
Este capítulo explica **como construir e interpretar itens dentro do W4D**. Ele não determina quais armas, armaduras, ferramentas, veículos, consumíveis ou artefatos existem em uma campanha. Essas listas pertencem ao cenário.

As regras apresentadas aqui fornecem uma linguagem comum para que cada cenário possa criar seus próprios equipamentos sem precisar desenvolver um sistema diferente para cada objeto.

> **Primeiro determine o que o item é. Depois determine o que sua própria natureza já permite fazer. Só então adicione mecânicas para aquilo que realmente precisa de uma regra.**
...
Todo item possui uma **Essência Narrativa**: aquilo que ele é dentro da ficção e tudo que naturalmente decorre disso.

Uma lanterna ilumina. Uma corda amarra e pode sustentar peso dentro do razoável. Um computador executa as funções esperadas de um computador naquele cenário. Uma espada corta. Um automóvel transporta pessoas e possui espaço para carga. Uma casa oferece abrigo, espaço e segurança compatíveis com sua construção.

Essas características não precisam ser transformadas em bônus ou efeitos individuais.
...
## 2. Anatomia de um Item

Um item pode apresentar apenas as informações necessárias para descrevê-lo. Entre os campos possíveis estão:

### Nome

Identifica o item dentro do cenário.

### Descrição

Explica o que o item é, sua aparência, funcionamento e qualquer informação narrativa necessária para compreender seu uso.

A descrição também estabelece sua Essência Narrativa.

### Preço

Indica seu valor em **Pilas**, quando possuir um preço relevante e puder ser comercializado.

O preço não determina disponibilidade. Um item pode possuir valor conhecido e ainda exigir licença, contato, autorização, localização específica ou simplesmente não estar à venda.

### Uso

Indica como o item é utilizado quando isso não for evidente por sua natureza ou quando sua ativação possuir alguma exigência mecânica específica.

### Efeito

Descreve qualquer resultado mecânico que o item produza além de sua Essência Narrativa.

Sempre que possível, itens especiais devem utilizar a mesma linguagem de Efeitos empregada pelas demais regras do W4D, evitando criar uma nova mecânica quando uma existente já representa aquilo que o item faz.
...
Quando relevante, indica a dificuldade ou progresso necessário para produzir o item através das regras de Fabricação.

### Propriedades Especiais

Qualquer característica que não seja explicada apenas pela Essência Narrativa pode ser descrita diretamente no item.

Uma propriedade especial deve dizer claramente **o que faz**, **quando funciona** e, quando necessário, **qual recurso utiliza**.

---

## 3. Itens Mundanos

Itens mundanos são objetos cujo funcionamento é explicado principalmente por sua própria natureza.

Martelos, lanternas, cordas, mochilas, computadores comuns, ferramentas, roupas, recipientes e inúmeros outros objetos normalmente não precisam conceder bônus apenas para justificar sua utilidade.

Se possuir a ferramenta correta torna uma ação possível, o item já está cumprindo sua função.

Exemplo:

> **Lanterna**  
> Ilumina uma área à frente enquanto estiver ligada.

Nada mais é necessário se bateria, alcance exato da luz ou resistência do objeto não forem importantes para aquele cenário.

---

## 4. Ferramentas

Ferramentas representam meios adequados para realizar determinadas tarefas.

A lógica básica é:

- **Ferramenta adequada:** condição normal para realizar a ação.
- **Improviso plausível:** a ação pode receber **Azar**.
- **Ausência de recurso indispensável:** a abordagem é impossível.

Uma ferramenta excepcional pode conceder **Sorte** ou outro efeito quando sua descrição disser explicitamente que oferece essa vantagem.

O bônus não existe apenas porque o objeto é uma ferramenta. Ele existe porque aquele equipamento específico é melhor que o necessário para executar a tarefa.

Exemplo:

> **Kit Forense Avançado**  
> Equipamento especializado para análise de vestígios. Quando suas ferramentas puderem ser utilizadas integralmente, concede **Sorte [1]** ao teste apropriado.
...
## 7. Consumíveis, Munição e Cargas

Alguns itens existem para serem gastos.

Poções, medpaks, granadas, munições especiais, componentes, baterias e outros consumíveis podem possuir uma quantidade definida de usos ou simplesmente desaparecer após sua utilização.

O item deve informar isso apenas quando for relevante.
...
Itens mágicos, tecnológicos, sobrenaturais ou extraordinários seguem as mesmas regras dos demais itens.

A origem narrativa muda; a estrutura mecânica não precisa mudar.

Uma espada encantada, um rifle de plasma, uma prótese experimental e uma relíquia divina podem possuir efeitos completamente diferentes, mas todos são construídos utilizando a mesma linguagem do sistema.

Sempre que um Efeito já existente representar adequadamente a capacidade do item, utilize esse Efeito em vez de criar uma regra exclusiva.

O item pode ainda possuir condições próprias de ativação, custo, recarga, restrições, requisitos ou consequências, desde que estejam descritos claramente.

Itens especiais não precisam estar disponíveis comercialmente. O cenário determina se são mercadorias, protótipos, relíquias, recompensas, heranças, tesouros ou objetos únicos.

---

## 9. Construindo um Item

Ao criar um equipamento para um cenário W4D, siga esta sequência:

1. **O que é o item?**  
   Defina sua natureza e função dentro do cenário.

2. **O que sua Essência Narrativa já resolve?**  
   Não crie mecânicas para aquilo que o bom senso já explica.

3. **O que realmente precisa de regra?**  
...
8. **Pode ser reparado ou produzido?**  
   Quando necessário, determine materiais, ferramentas, Perícias e condições adequadas.

Ao final, remova qualquer informação que não ajude a compreender ou utilizar o item.

> **Um bom item possui todas as regras de que precisa e nenhuma regra de que não precisa.**

---

## 10. Exemplos de Estrutura

Os exemplos abaixo demonstram apenas formatos possíveis. Seus valores não representam uma lista universal de equipamentos.
...
> **Poção de Cura / Medpak**  
> **Uso:** consumível  
> **Efeito:** aplica o efeito de recuperação determinado pelo cenário. Após o uso, o item é consumido.
