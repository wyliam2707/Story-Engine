...
Medicina [Único]; Alquimia [Excelente] no Saber e no Poder; Herborismo [Excelente]; Técnicas de Cultivo [Excelente]; Corpo do Dragão [Único]; Negociação e Diplomacia [Padrão]; Compreensão [Padrão]; Lótus de Fogo, Gelo e Madeira [Excepcional]; Arte do Dragão [Único]. Esses graus não alteram os domínios aprovados nem constituem um sistema de testes. Santo e a progressão 1, 2, 4, 8, 16, 36, 64 pertencem ao cultivo do mundo de origem. A comparação 16/64 de Sana e Will estabelece diferença de escala, não equivalência automática entre os sistemas dos dois mundos.

## Integridade e etapas pendentes

As referências visuais de Will e Sana foram fornecidas na conversa de criação, mas não foram incorporadas ao repositório. Não inventar caminhos de imagem nem substituir as referências. Não há Ficção iniciada, resultados do encontro estabelecidos ou condições de ferimento detalhadas. Após as fichas necessárias, seguir para Autoridades, Direção, Políticas, Mundo Necessário, Estado Inicial, Auditoria e START. Preservar a autoria do Diretor e as Cadeiras independentes.
